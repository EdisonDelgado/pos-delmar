...
class SaleNoteController extends Controller
{
    ...
    public function saleReport(Request $request){
        ...
        $sales = DB::table('sale_notes')
            ->whereDay('updated_at','=', Carbon::today(-3)->day)
            ->get();
        $salesMonth = DB::table('sale_notes')
            ->whereMonth('updated_at','=', Carbon::today(-3)->month)
            ->get();
        $total = 0;
        $totalMonth = 0;
        foreach($salesMonth as $sm){
            $totalMonth = $sm->amount +$totalMonth;
        }
        foreach ($sales as $sale){
            $total = $sale->amount + $total;
        }
        return view('reports.sales',['sales'=> $sales, 'salesMonth'=>$salesMonth,'totalMonth'=>$totalMonth ,'totalDay'=>$total]);
